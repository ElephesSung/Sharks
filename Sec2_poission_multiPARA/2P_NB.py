...
class insilico_experiment:
    def __init__(self, kill_lambda_avg=3.0, kill_lambda_sd=1.0, Num_killers_obs=300, random_seed=66):
...
def plot_synthetic_vs_jointcorner(
    ...
    colors = cmap(np.linspace(0.3, 0.9, len(lambda_avg_list)))

    if parameter_display is None:
        parameter_display = {"mu_lambda": r"$\mu_\lambda$", "sigma_lambda": r"$\sigma_\lambda$"}
    ...
    conditions = [
        fr"$\mu_\lambda={lam}$, $\sigma_\lambda={lambda_sd_list[i]}$"
        for i, lam in enumerate(lambda_avg_list)
    ]
    ...
    gs_corner = gridspec.GridSpecFromSubplotSpec(
        2, 2, subplot_spec=gs[0, 1], width_ratios=[4, 1.2], height_ratios=[1.2, 4], wspace=0.05, hspace=0.05
    )
...
def main(No_cell):
    ...
    lambda_avg_list = [0.5, 1.2, 2.0]
    lambda_sd_list = [0.2, 0.5, 1.0]
    cell_num_list = [No_cell] * len(lambda_avg_list)

    experiment = {}
    for i, lam_avg in enumerate(lambda_avg_list):
        experiment[i] = insilico_experiment(
            kill_lambda_avg=lam_avg, kill_lambda_sd=lambda_sd_list[i], Num_killers_obs=cell_num_list[i], random_seed=i+50
        )

    conditions = [f"μλ={lambda_avg_list[i]}, σλ={lambda_sd_list[i]}" for i in range(len(lambda_avg_list))]
    ...
    plot_synthetic_vs_jointcorner(
        ...
        lambda_avg_list=lambda_avg_list,
        lambda_sd_list=lambda_sd_list,
        idatas=idatas,
        ground_truth_values=ground_truth_values,
        save_path=f"./syn_results/ANA_{No_cell}.pdf",
    )
